...
namespace Ex03.GarageLogic
{
    public class Garage
    {
        ...
        //electric car
        public string AddElectricCar(string i_CustomerName, string i_OwnerPhone, string i_ModelName, string i_LicenseNumber, string i_CurrentEnergy, string i_CarColor, string i_NumOfDoors, string i_CurrnetAirPresure)
        {
            float currentEnergy = float.Parse(i_CurrentEnergy);
            int intNumOfDoors = int.Parse(i_NumOfDoors);
            int CurentAirPreasur = int.Parse(i_CurrnetAirPresure);
            ...
            foreach(CustomerCard customerCard in m_CustomerCards)
            {
                if(customerCard.OwnerName == i_CustomerName)
                {
                    ...
                    {
                        customerCard.VehicleList.Add(new Car(i_ModelName, i_LicenseNumber, currentEnergy, 5.2f, CarColor, numOfDoors, CurentAirPreasur));
                    }
                    else
                    {
        ...
        //fule car
        public string AddFuleCar(string i_OwnerName, string i_OwnerPhone, string i_ModelName, string i_LicenseNumber, string i_CurrentEnergy, string i_CarColor, string i_NumOfDoors, string i_CurrnetAirPresure)
        {
            float currentEnergy = float.Parse(i_CurrentEnergy);
            int intNumOfDoors = int.Parse(i_NumOfDoors);
            int CurentAirPreasur = int.Parse(i_CurrnetAirPresure);
            ...
            foreach (CustomerCard customerCard in m_CustomerCards)
            {
                 if (Enum.TryParse<eCarColor>(i_CarColor, true, out eCarColor CarColor) && Enum.TryParse<eNumOfDoors>(i_NumOfDoors, true, out eNumOfDoors numOfDoors))
                 {
                    ...
                    { 
                        customerCard.VehicleList.Add(new Car(i_ModelName, i_LicenseNumber, currentEnergy, 44, eFuelType.Octan95, CarColor, numOfDoors, CurentAirPreasur));
                    }
                 }
                 else
                 {
        ...
        public string AddEclectricMotorcycle(string i_OwnerName, string i_OwnerPhone, string i_ModelName, string i_LicenseNumber, string i_CurrentEnergy,
            string i_LicenseType, string i_EngineCapacityInCubicCentimeter, string i_CurrnetAirPresure)
        {
            float currentEnergy = float.Parse(i_CurrentEnergy);
            ...
            foreach (CustomerCard customerCard in m_CustomerCards)
            {
                if (customerCard.OwnerName == i_OwnerName)
                {
                    if (Enum.TryParse<eLicenseType>(i_LicenseType, true, out eLicenseType LicenseType))
                    {
                        ...
                        LicenseType, engineCpacity, CurentAirPreasur));
                    }
                    else
                    {
        ...
        //fule truck
        public string AddTruck(string i_OwnerName, string i_OwnerPhone, string i_ModelName, string i_LicenseNumber, string i_CurrentEnergy, string i_RefrigeratedTruck, string i_CargoVolume, string i_CurrnetAirPresure)
        {

            float currentEnergy = float.Parse(i_CurrentEnergy);
            bool refrigeratedTruck = false;
            if (i_RefrigeratedTruck == "Y")
            {
                refrigeratedTruck = true;
            }
            int CurentAirPreasur = int.Parse(i_CurrnetAirPresure);
            ...
            foreach (CustomerCard customerCard in m_CustomerCards)
            {
                if (customerCard.OwnerName == i_OwnerName)
                {
                    customerCard.VehicleList.Add(new Truck(i_ModelName, i_LicenseNumber, currentEnergy, 130, eFuelType.Soler, refrigeratedTruck, cargoVolume, CurentAirPreasur));
                }
            }
